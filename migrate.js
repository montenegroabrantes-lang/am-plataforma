...
try {
  ...
  // 17. novas_movimentacoes em sync_execucoes — total de movimentações salvas por execução
  await db.execute(`ALTER TABLE sync_execucoes ADD COLUMN IF NOT EXISTS novas_movimentacoes INTEGER NOT NULL DEFAULT 0`).catch(() => {});

  // 18. jusbrasil_monitorado — controla quais processos foram registrados na API JusBrasil
