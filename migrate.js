...
// 2. Cria ou atualiza o usuário master usando MASTER_EMAIL + MASTER_SENHA
const nome  = process.env.MASTER_NOME  || 'Ramona';
const email = process.env.MASTER_EMAIL;
const senha = process.env.MASTER_SENHA;

if (!email || !senha) {
  console.log('[migrate] ⚠️  MASTER_EMAIL ou MASTER_SENHA não configurados no Railway — defina essas variáveis');
} else {
