...
export async function recarregarAiConfig(db) {
  try {
    ...
    console.log(`[AI Config] Diagnóstico=${aiConfig.roteamento.diagnostico} | Peças=${aiConfig.roteamento.peticao} | Claude=${aiConfig.claude.apiKey ? 'chave OK' : 'sem chave'} | OpenAI=${aiConfig.openai.apiKey ? 'chave OK' : 'sem chave'}`);
  } catch (err) {
